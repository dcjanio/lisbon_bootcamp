module six::six {
    ...
    struct StakedSix has key, store {
        id: UID,
        amount: u64
    }
    ...
    fun init(witness: SIX, ctx: &mut TxContext) {
        ...
        let _icon = url::new_unsafe(ascii::string(b"https://six.com"));
    ...
    public fun swap_sui_for_six(
        ...
        let fee_coin = coin::from_balance<SUI>(fee_balance, ctx);
    ...
    public fun burn_six_for_sui(
        ...
        let gross_sui_coin = coin::from_balance<SUI>(gross_sui_balance, ctx);
    ...
    public fun stake(
        ...
        let six_balance = coin::into_balance(six_coin);
        balance::join(&mut manager.staked_coins, six_balance);
        
        // Create a StakedSix object
        let staked_six = StakedSix {
            id: object::new(ctx),
            amount: six_value
        };
        transfer::public_transfer(staked_six, tx_context::sender(ctx));
    }

    public fun unstake(
        manager: &mut CoinManager,
        staked_six: StakedSix,
        ctx: &mut TxContext
    ) {
        let StakedSix { id, amount } = staked_six;
        object::delete(id);
        
        assert!(balance::value(&manager.staked_coins) >= amount, EInsufficientStake);
        
        // Return the original SIX tokens
        let six_balance = balance::split(&mut manager.staked_coins, amount);
        let six_coin = coin::from_balance(six_balance, ctx);
